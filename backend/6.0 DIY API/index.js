...
app.get("/random", (req, res) => {
  ...
  var randomJoke = jokes[randomId];
  res.json((randomJoke));
});
//2. GET a specific joke
app.get("/jokes/:id", (req, res) => {
  var jokeId = parseInt(req.params.id);
  var foundjoke = jokes.find(joke => joke.id === jokeId);
  res.json(foundjoke);
});
//3. GET a jokes by filtering on the joke type
app.get("/filter", (req, res) => {
  var type = req.query.type;
  var foundjokes = jokes.filter(joke => joke.jokeType === type);
  res.json(foundjokes);
});
//4. POST a new joke
...
app.delete("/jokes/:id", (req, res) => {
  var jokeId = parseInt(req.params.id);
  var foundId = jokes.findIndex((joke) => joke.id === jokeId);
  if (foundId > -1) {
